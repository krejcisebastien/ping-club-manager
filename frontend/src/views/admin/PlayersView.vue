...
<template>
  <AppLayout title="Joueurs" :nav-links="navLinks">
    ...
    <DataTable :value="players" :loading="loading" class="bg-white rounded-xl shadow-sm overflow-hidden" striped-rows @row-click="router.push(`/admin/players/${$event.data.id}`)">
      <template #empty>
